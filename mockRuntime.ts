...
export class MockRuntime extends EventEmitter {
	...
	public async start(program: string, stopOnEntry: boolean, noDebug: boolean): Promise<void> {
		console.log('entered start execution function');
		const axios = require('axios');
		await axios.get('http://localhost:5000/emu?session=1234')
	...
	public continue(reverse: boolean) {
		console.log('entered continue loop');
		if (reverse) {
	 ...
	 */
	public async step(instruction: boolean, reverse: boolean, event = 'stopOnStep') {
			
		// fetch('http://localhost:5000/step')
    	// 	.then(res => res.text())
    	// 	.then(text => console.log(text));

		const axios = require('axios');
		await axios.get('http://localhost:5000/step')
			.then(function (response) {
			  // handle success
			  console.log(response);
			})
